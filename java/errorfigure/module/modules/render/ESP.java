...
public class ESP
    ...
    public Mode<Enum> mode = new Mode("Mode", "mode", (Enum[])ESPMode.values(), (Enum)ESPMode.True2D);
    ...
    public void onScreen(EventRender2D eventRender) {
        if (mode.getValue() == ESPMode.True2D) {
...
//                            RenderUtils.drawRect(x + 0.5, y + 0.5, endx - 0.5, endy - 0.5, color);
//                            RenderUtils.drawRect(x - 0.5, y - 0.5, endx + 0.5, endy + 0.5, getColor(0, 150));
                            RenderUtils.drawBorderedRect((float) (x + 1.5), (float) (y + 1.5), (float) (endx - 1.5), (float) (endy - 1.5), 1, new Color(255, 255, 255).getRGB(), getColor(255, 80));
                        }
                        if (HEALTH.getValue()) {
                            ...
                            Gui.drawRect((float) (x - 5.5), (float) (y - 1.0), (float) (x - 3.5), (float) endy,
                                    getColor(170, 255));
                            Gui.drawRect(x - 5.5, endy - 1.0, x - 3.5, healthLocation - 2, new Color(255, 255, 255).getRGB());
            ...
            GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
            GlStateManager.popMatrix();
            RenderUtils.drawRect(0.0, 0.0, 0.0, 0.0, -1);
        }
    }
    ...
    private void onRender(EventRender3D eventRender) {
        if(mode.getValue() == ESPMode.True2D) {
            try {
                updatePositions(eventRender.getPartialTicks());
            } catch (Exception ex) {
            }
        }
    }
    ...
    private void box(float left, float top, float right, float bottom) {
        ...
        RenderUtils.drawLine(left, top, right, top, 2.0f);
        RenderUtils.drawLine(left, bottom, right, bottom, 2.0f);
        RenderUtils.drawLine(left, top, left, bottom, 2.0f);
        RenderUtils.drawLine(right, top, right, bottom, 2.0f);
        RenderUtils.drawLine(left + 1.0f, top + 1.0f, right - 1.0f, top + 1.0f, 1.0f);
        RenderUtils.drawLine(left + 1.0f, bottom - 1.0f, right - 1.0f, bottom - 1.0f, 1.0f);
        RenderUtils.drawLine(left + 1.0f, top + 1.0f, left + 1.0f, bottom - 1.0f, 1.0f);
        RenderUtils.drawLine(right - 1.0f, top + 1.0f, right - 1.0f, bottom - 1.0f, 1.0f);
        RenderUtils.drawLine(left - 1.0f, top - 1.0f, right + 1.0f, top - 1.0f, 1.0f);
        RenderUtils.drawLine(left - 1.0f, bottom + 1.0f, right + 1.0f, bottom + 1.0f, 1.0f);
        RenderUtils.drawLine(left - 1.0f, top + 1.0f, left - 1.0f, bottom + 1.0f, 1.0f);
        RenderUtils.drawLine(right + 1.0f, top - 1.0f, right + 1.0f, bottom + 1.0f, 1.0f);
    ...
    private void health(EntityLivingBase entity, float left, float top, float right, float bottom) {
        ...
        RenderUtils.drawLine(left - 5.0f, top + height * (1.0f - healthPercent) + 1.0f, left - 5.0f, bottom, 2.0f);
    }
