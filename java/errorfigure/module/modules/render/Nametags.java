...
extends Module {
    ...
    private void drawNames(EntityPlayer player) {
        ...
        float nw = - (float)this.getWidth(this.getPlayerName(player)) / 2f;
        float offset = this.getWidth(this.getPlayerName(player)) + 4;
        RenderUtils.drawBorderedRect(nw - 5, -3.0f, width - 5, 10.0f, 1.0f, new Color(255, 255, 255).getRGB(), new Color(255, 255, 255, 170).getRGB());
        GlStateManager.disableDepth();
        this.drawString(this.getPlayerName(player), w - offset, 0.0f, new Color(50, 50, 50).getRGB());
        if (player.getHealth() == 10.0f) {
            int n = 16776960;
        }
        int color = player.getHealth() > 10.0f ? RenderUtils.blend(new Color(-16711936), new Color(-256), 1.0f / player.getHealth() / 2.0f * (player.getHealth() - 10.0f)).getRGB() : RenderUtils.blend(new Color(-256), new Color(-65536), 0.1f * player.getHealth()).getRGB();
        GlStateManager.enableDepth();
    }

    private void drawString(String text, float x, float y, int color) {
        mc.fontRendererObj.drawString(text, (int) (x - 6), (int) (y), color);
    }
